...
function log(...args: any[]) {
	console.log('auto review log:', ...args);
}

const owner = process.env.GITHUB_REPO_OWNER;
const repo = process.env.GITHUB_REPO
const pullNumber = Number(process.env.GITHUB_PULL_NUMBER || '0')
...
async function generateReviewComment(changedFiles: any, commits: any) {
...
async function bootstrap() {
	...
	generateReviewComment(changedFiles, commits);
}
